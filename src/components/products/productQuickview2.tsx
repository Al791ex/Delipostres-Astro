...
interface Props {
  ...
  sizes: Map<string,number> | undefined;
}
                      ...
                      <div className="row mb-2">
                        ...
                        <div className="d-flex flex-wrap text-center my-4">
                          {sizes && Array.from(sizes).map((size) => (
                            <div key={size[0]} className="mb-3 me-3">
                              <div className="form-check">
                                <input className="form-check-input rounded-2" type="radio" name="flexRadioDefault" id={`input${size[0]}`} />
                                <label className="cursor-pointer" htmlFor={`input${size[0]}`}>{size[1]}</label>
                              </div>
                            </div>
                          ))}
                        </div>
